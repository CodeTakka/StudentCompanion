...
async function initAssessments() {
  try {
    ...
    const options = [
      ...
      ...courses.map(
        (c) =>
          `<option value="${c._id}">${escapeHtml(c.code)} — ${escapeHtml(c.name)}</option>`,
      ),
    ];
...
function getEarnedMarks(assessment) {
  return assessment.earnedMarks !== null
...
function rowClass(a) {
  ...
  if (!submitted && a.dueDate && new Date(a.dueDate) < new Date())
    return "overdue-row";
  return "";
}
  ...
  const filtered =
    selectedCourse === "all"
      ...
      : allAssessments.filter(
          (a) => a.courseId && a.courseId._id === selectedCourse,
        );
...
window.openSubmissionModal = function (
  ...
  if (completed === "true" || completed === true) {
    statusDiv.textContent =
      "This assessment has already been submitted. Uploading again will resubmit.";
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    if (!file) {
      console.log("NO FILE — calling apiSubmitAssessmentWithoutFile");
      const confirmNoFile = confirm(
        "You did not attach a file.\n\nDo you want to submit without a file?",
      );
      if (!confirmNoFile) {
      ...
      // Submit WITHOUT file
      try {
        showSubmissionStatus("Submitting...", "info");

        await apiSubmitAssessmentWithoutFile(currentAssessmentId);

        showSubmissionStatus(
          "✓ Assessment submitted without a file!",
          "success",
        );
    ...
    if (!allowedTypes.includes(file.type)) {
      showSubmissionStatus(
        "File type not allowed. Use PDF, Word, Excel, Text, or Images.",
        "error",
      );
      return;
    }
