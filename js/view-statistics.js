async function loadStats() {
  try {
    ...
    document.getElementById("statAssessments").textContent =
    ...
    const averages = await Promise.all(
      courses.map((c) =>
        apiGetCourseAverageGlobal(c._id).catch(() => ({ average: null })),
      ),
    );
    ...
    document.getElementById("statOverallAverage").textContent = averages.length
      ? Math.round(
          averages
            .filter((a) => a.average !== null)
            .reduce((s, a) => s + a.average, 0) /
            averages.filter((a) => a.average !== null).length,
        ) + "%"
      : "N/A";
  } catch (err) {
